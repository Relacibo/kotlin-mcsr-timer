    ...
    Box(Modifier.width(400.dp).padding(vertical = 15.dp, horizontal = 20.dp)) {
        Column(modifier = Modifier.fillMaxHeight(), verticalArrangement = Arrangement.spacedBy(15.dp)) {
            ...
            Select(
                ...
                state = preferences.updatesPerSecond,
                setState = { preferences = preferences.copy(updatesPerSecond = it) },
                values = listOf(
                    Pair("15", UpdatesPerSecond.N15),
                    Pair("30", UpdatesPerSecond.N30),
                    Pair("60", UpdatesPerSecond.N60),
                    Pair(
                        "144", UpdatesPerSecond.N144
                    )
                ),
            )
        }
